...
from zerotwobot import telethn as SaitamaTelethonClient, TIGERS, DRAGONS, DEMONS
...
def info(update: Update, context: CallbackContext):
    ...
    rep = message.reply_text(
        "<code>Gathering info frm the crew...</code>", parse_mode=ParseMode.HTML
    )
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\nThe Disaster level of this person is 'SwordsMan'."
        disaster_level_present = True
    elif user.id in DRAGONS:
        text += "\n\nThe Disaster level of this person is 'Cook'."
        disaster_level_present = True
    elif user.id in DEMONS:
        text += "\n\nThe Disaster level of this person is 'Navigator'."
        disaster_level_present = True
    elif user.id in TIGERS:
        text += "\n\nThe Disaster level of this person is 'Sniper'."
        disaster_level_present = True
    elif user.id in WOLVES:
        text += "\n\nThe Disaster level of this person is 'musician'."
        disaster_level_present = True

    if disaster_level_present:
        text += ' [<a href="https://t.me/NikaUpdates/4">?</a>]'.format(bot.username)
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == message.from_user.id:
            message.reply_text(
                "nigga u cant set ur own bio! You're at the mercy of others here..."
            )
            return
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "Erm... yeah, I only trust the straw hat crew to set my bio."
            )
            return
...
*What is that health thingy?*
 Come and see [HP System explained](https://t.me/NikaUpdates/5)
"""
